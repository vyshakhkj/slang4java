...
public class BinaryPlus extends Exp {
	...
	public SYMBOL_INFO evaluate(RUNTIME_CONTEXT cont) {
		...
		if(eval_left.getType() == TYPE_INFO.TYPE_STRING && eval_right.getType() == TYPE_INFO.TYPE_STRING){
			SYMBOL_INFO ret_val = new SYMBOL_INFO();
			ret_val.setStr_val(eval_left.getStr_val() + eval_right.getStr_val());
			ret_val.setType(TYPE_INFO.TYPE_STRING);
			ret_val.setSymbolName("");
			return ret_val;
		} else if(eval_left.getType() == TYPE_INFO.TYPE_NUMERIC && eval_right.getType() == TYPE_INFO.TYPE_NUMERIC){
			SYMBOL_INFO ret_val = new SYMBOL_INFO();
